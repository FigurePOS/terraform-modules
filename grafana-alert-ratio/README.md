...
`interval` is the lookback window in seconds (default `300` = 5m). Rules evaluate every 2m, pending 2m, `KeepLast` on query error. Grafana provider (`url` / `auth`) is minted by `fgr tf` / `auth-terraform-providers`.
