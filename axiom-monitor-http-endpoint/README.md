...
Needs the Axiom provider in the service root (`api_token`). Eval every 2m over a 15m lookback (`interval` seconds, default `900`; min 15m because Axiom rejects `bucket to 1m` below that). Latency is `avg` of 1m p95s (Datadog `avg(last_Xm):p95`), not p95 of the whole window. Fires after 2 consecutive evals.
