...
Needs the Axiom provider in the service root (`api_token`). Eval every 2m over a 10m lookback (`interval` seconds, default `600` = Datadog `avg(last_10m)`). Query is a window average, not “any 1m point”. Fires after 2 consecutive evals.
