...
resource "datadog_monitor" "sqs_number_of_messages_monitor" {
  ...
  type    = "metric alert"
  message = "{{#is_alert}}${local.oncall_handle}{{/is_alert}} {{#is_alert_recovery}}${local.oncall_handle}{{/is_alert_recovery}} ${local.slack_warning_handle} ${var.total_number_message}"
...
resource "datadog_monitor" "sqs_number_of_messages_dead_letter_monitor" {
  ...
  type    = "metric alert"
  message = "${local.slack_warning_handle}"
