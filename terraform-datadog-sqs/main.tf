...
resource "datadog_monitor" "sqs_number_of_messages_monitor" {
  ...
  type = "metric alert"
  message = var.message
  query = "max(last_5m):avg:aws.sqs.approximate_number_of_messages_visible{queuename:${lower(var.queue_name)},env:${var.env}} > ${var.queue_messages_critical}"
  monitor_thresholds {
...
resource "datadog_monitor" "sqs_number_of_messages_dead_letter_monitor" {
  ...
  type = "metric alert"
  message = var.message
  query = "max(last_5m):avg:aws.sqs.approximate_number_of_messages_visible{queuename:${lower(var.dead_letter_queue_name)},env:${var.env}} > ${var.dead_letter_queue_messages_critical}"
  monitor_thresholds {
